...
/*
    In the above code we first create an fd_set variable readfds, which monitors all the clients’ active file descriptors
    as well as the active file descriptors on the main servers listening socket. For an old client sending data, readfds
    would already be activated and thus we’ll check in the existing list to see which client has sent the data.
    When a new client connects to the server, master-sock will be activated and a new file descriptor will be open for
    that particular client. We store this file descriptor in a client_list and add it to the readfds variable in the next
    iteration to monitor the activity from the client.
*/
